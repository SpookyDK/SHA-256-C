...
int pad_message(const u_char *mymessage, const uint32_t message_len, u_char **out, uint32_t *outlen, uint32_t *outchunks) {
    ...
    char *padded_message = calloc(1, nearest_byte);
...
int process_chunk(u_char *const padded_message_chunk, uint32_t chunk_len, uint32_t *h_values) {
    ...
    // THe chunk is 16 words which we copy into the first 16 words of our array in big endian bullshit
    for (int i = 0; i < 16; i++) {
...
int sha256_hash(const u_char *mymessage, const uint32_t message_len, char **out, uint32_t *outlen) {
    ...
    for (int i = 0; i < chunks; i++) {
        ...
        // we add our final working variables to the constant h values and contionue
    }
